...
clc
 ...
 boundary_PHI= ((y-(-pi)).*(y-(-pi))).*sin( (pi/2) * ((y-(-pi))/(pi-(-pi)))  );  % goes on U(1,:)
  boundary_PSI= ( (cos(pi.*(y-(-pi)))-1 ).*cosh(pi-y));%goes on U(Nodes+2,:)
 % boundary PSI updated  by directive from professor's email.
 F= zeros(Nodes+2);
 for j=2:Nodes+1
     for i=2:Nodes+1
...
%*******Uncomment to print out plot showing results of either solver******
...
% at Neumann, given derivatives, but will have to solve for U on both 
% edges
%     Moon=((3*(-pi)^2)*((2*pi+pi)^3)   -3*((-pi)^3)*((2*pi+pi)^2)); %solve for at y=-pi
%     Sun = (  (3*(pi^2)*((pi)^3)   -3*(pi^3)*((pi)^2))  ); %solves for at y=pi
%         dV_zero=     Moon.*((2*pi-x).^3) .* (x .^3);  % solving for both Neumann
%         dV_N_PLUS=   Sun .*((2*pi-x).^3 .*(x .^3)); % BCs
% 
% % since now dv/dy not necessary equal to zero, will need value of dx*2....
% two_dx= 2*dx;
...
%**************************************************************************
%**************************************************************************
%****************END of Gauss-Seidel To test MMS **************************
%**************************************************************************
%**************************************************************************


%**************************************************************************
%********************Start of SOR to test Manufactured*********************
%**************************************************************************
%**************************************************************************
%  condition_SOR =0;
%  counter_SOR=0;
%  beta= 1.5; % recommended value for successive over relaxation
%  alpha= beta/4; % put at multiplier to save some time, is omega
%  gamma = 1-beta; % is 1-omega according to SOR
%   while ( condition_SOR ==0) %Iteration number 
%        for i=2:Nodes+1%must solve for U in here as Ditchelt B.C not specified- U will be unknown here
%          
%           U(i,1)=U(i,1)*gamma +( U(i-1,1) + 2*U(i,2)-dV_zero(i)*two_dx + U(i+1,1) - dx2*Q(i,1) )*alpha; 
%         
%        end
%        
%        for j=2:Nodes+1
%         for i=2:Nodes+1 % U at boundary given, dont have to solve for them
%            U(i,j)= U(i,j)*gamma +( U(i-1,j) + U(i,j+1) + U(i+1,j) + U(i,j-1) - dx2*Q(i,j) )*alpha;
%                
%         end
%        end
%        
%        for i=2:Nodes+1 %must solve for U in here as Ditchelt B.C not specified- U will be unknown here
%              
%            U(i,Nodes+2)= U(i,Nodes+2)*gamma+( U(i-1,Nodes+2) + U(i+1,Nodes+2) +dV_N_PLUS(i)*two_dx + 2*U(i,Nodes+1) - dx2*Q(i,j) )*alpha; %made two additional for loops rather than having
%              % only one main for loop for i & a bunch of if statements that
%              % had to be checked every time
%        
%        end
...
%  counter_SOR
 %************************************************************************
 %********************END of SOR to test Manufactured*********************
 %************************************************************************
 %************************************************************************
...
% uncomment to test either SOR or GS vs the manufactured solution
%  U_Plot = transpose(U);
% V_Plot = transpose(V);
...
% %uncomment to test a 'slice'
% n= randi([2,Nodes+1],1) % will create a random number to test the 'slices'
%  plot(x,U_Plot(n,:))
%  hold on
%  plot(x,V_Plot(n,:), '--')
%  title( 'Slice Comparison for Manufactured Solution')
%  legend(' Numerical', 'Manufactured')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
